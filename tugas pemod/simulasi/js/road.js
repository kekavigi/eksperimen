...
function road(roadID, roadLen, laneWidth, nLanes, traj_x, traj_y,
  ...
  this.inVehBuffer =
  0.9; // number of waiting vehicles; if>=1, updateBCup called
  this.iTargetFirst =
  0; // set by getTargetNeighbourhood: first veh in defined region
...
road.prototype.initRegularVehicles = function(densityPerLane, fracTruck) {
  ...
  for (var i = 0; i < nvehPlus; i++) {
    ...
    var lane = i % this.nLanes; // left: 0; right: nLanes-1
    var fracTruckRight = Math.min(this.nLanes * fracTruck, 1);
    var fracTruckRest = (this.nLanes * fracTruck > 1) ? ((this.nLanes *
      fracTruck - 1) / (this.nLanes - 1)) : 0;
    var fracTruck = (lane === this.nLanes - 1) ? fracTruckRight :
      fracTruckRest;
    var vehType = (Math.random() < fracTruck) ? "truck" : "car";
    var vehLength = (vehType === "car") ? car_length : truck_length;
    var vehWidth = (vehType === "car") ? car_width : truck_width;
    ...
    if ((sLead > 2) && (sFollow > 2)) {
      vehPlus[iveh] = new vehicle(vehLength, vehWidth, u, lane, 0.8 *
        speedInit, vehType); // IC
      iveh++;
    }
  }
...
road.prototype.gridTrajectories = function(traj_xExt, traj_yExt) {
  ...
  this.traj_x = function(u) {
    ...
    if (false) {
      ...
      console.log("in road.gridTrajectories def this.traj_x: u=", u,
        " roadLen=", this.roadLen, " this.nSegm=", this.nSegm, " iLower=",
        iLower, " iUpper=", iUpper, " this.xtab[iLower]=", this.xtab[
          iLower], " this.xtab[iUpper]=", this.xtab[iUpper]);
    }
  ...
  if (false) {
    console.log("end road.gridTrajectories: this.nSegm=", this.nSegm,
      " this.xtab[0]=", this.xtab[0], " this.xtab[1]=", this.xtab[1],
      " this.traj_x(0)=", this.traj_x(0), " this.xtab[this.nSegm]=", this
      .xtab[this.nSegm], " this.traj_x(this.roadLen)=", this.traj_x(this
        .roadLen));
  }
  if (false) {
    var utab = [];
    ...
    for (var i = 1; i <= this.nSegm; i++) {
      utab[i] = utab[i - 1] + Math.sqrt(Math.pow(this.xtab[i] - this.xtab[i -
        1], 2) + Math.pow(this.ytab[i] - this.ytab[i - 1], 2));
      console.log("i=", i, " utabOld=", parseFloat(this.roadLen * i / this
          .nSegm).toFixed(1), " utab=", parseFloat(utab[i]).toFixed(1),
        " xtabOld=", parseFloat(this.xtabOld[i]).toFixed(1), " xtab=",
...
road.prototype.update_nSegm_tabxy = function() {
  ...
  var nSegmNew = Math.round(nSegm_per_rad * phiabsCum + nSegm_per_m * this
    .roadLen);
  ...
  if (false) {
    console.log("end road.update_nSegm_tabxy: new this.nSegm=", this.nSegm,
      " this.traj_y(0)=", this.traj_y(0), " this.traj_y(this.roadLen)=",
      this.traj_y(this.roadLen));
  }
  if (false) {
...
road.prototype.writeVehicles = function(umin, umax) {
  console.log("\nin road.writeVehicles(): itime=", itime, " roadID=", this
    .roadID, " nVehicles=", this.veh.length, " roadLen=", this.roadLen);
  ...
  for (var i = 0; i < this.veh.length; i++) {
    if ((this.veh[i].u >= uminLoc) && (this.veh[i].u <= umaxLoc)) {
      console.log(" veh[" + i + "].id=" + this.veh[i].id + "  type=" + this
        .veh[i].type + "  len=" + this.veh[i].length + "  u=" + parseFloat(
          this.veh[i].u, 10).toFixed(1) + "  lane=" + this.veh[i].lane +
        "  v=" + parseFloat(this.veh[i].v, 10).toFixed(1) + "  speed=" +
        parseFloat(this.veh[i].speed, 10).toFixed(1) + "  acc=" +
        parseFloat(this.veh[i].acc, 10).toFixed(1) + "  iLead=" + this.veh[
          i].iLead + "  iLag=" + this.veh[i].iLag + "  iLeadRight=" + this
        .veh[i].iLeadRight + "  iLagRight=" + this.veh[i].iLagRight +
        "  iLeadLeft=" + this.veh[i].iLeadLeft + "  iLagLeft=" + this.veh[i]
        .iLagLeft + "");
    }
  }
} // writeVehicles
...
road.prototype.writeVehiclesSimple = function(umin, umax) {
  console.log("\nin road.writeVehiclesSimple(): roadID=", this.roadID,
    " nveh=", this.veh.length, " nLanes=", this.nLanes, " itime=", itime);
  var uminLoc = (typeof umin !== 'undefined') ? umin : 0;
  var umaxLoc = (typeof umax !== 'undefined') ? umax : this.roadLen;
  for (var i = 0; i < this.veh.length; i++) {
    if ((this.veh[i].u >= uminLoc) && (this.veh[i].u <= umaxLoc)) {
      console.log(" veh[" + i + "].type=" + this.veh[i].type + "  id=" + this
        .veh[i].id + "  u=" + parseFloat(this.veh[i].u, 10).toFixed(1) +
        "  v=" + parseFloat(this.veh[i].v, 10).toFixed(1) + "  lane=" + this
        .veh[i].lane + "  dvdt=" + parseFloat(this.veh[i].dvdt, 10).toFixed(
          2) + "  speed=" + parseFloat(this.veh[i].speed, 10).toFixed(1) +
...
road.prototype.writeSpeedlimits = function(umin, umax) {
  console.log("\nin road.writeSpeedlimits(): roadID=", this.roadID, " nveh=",
    this.veh.length, " nLanes=", this.nLanes, " itime=", itime);
  var uminLoc = (typeof umin !== 'undefined') ? umin : 0;
  var umaxLoc = (typeof umax !== 'undefined') ? umax : this.roadLen;
  for (var i = 0; i < this.veh.length; i++) {
    if ((this.veh[i].u >= uminLoc) && (this.veh[i].u <= umaxLoc)) {
      if (this.veh[i].isRegularVeh()) {
        console.log(" veh[" + i + "].type=" + this.veh[i].type + " id=" + this
          .veh[i].id + " type=" + this.veh[i].type + " u=" + parseFloat(this
            .veh[i].u, 10).toFixed(1) + " speedlimit_kmh=" + formd0(3.6 *
            this.veh[i].longModel.speedlimit) + " speed=" + formd(this.veh[
            i].speed) + " v0=" + formd(this.veh[i].longModel.v0) + " acc=" +
...
road.prototype.writeVehiclesIDrange = function(idmin, idmax) {
  ...
  for (var i = 0; i < this.veh.length; i++) {
    if ((this.veh[i].id >= idmin) && (this.veh[i].id <= idmax)) {
      var s = (i == 0) ? 10000 : this.veh[i - 1].u - this.veh[i - 1].length -
        this.veh[i].u;
      console.log("t=", parseFloat(dt * itime).toFixed(1), " veh", i,
        " type=", this.veh[i].type, " id=", this.veh[i].id, " roadID=", this
        .roadID, " u=", parseFloat(this.veh[i].u, 10).toFixed(3),
        //" v=",parseFloat(this.veh[i].v,10).toFixed(1),
        //" lane=",this.veh[i].lane,
        //" dvdt=",parseFloat(this.veh[i].dvdt,10).toFixed(2),
        " s=", parseFloat(s).toFixed(3), " speed=", parseFloat(this.veh[i]
          .speed, 10).toFixed(3), " acc=", parseFloat(this.veh[i].acc, 10)
        .toFixed(3), "");
    }
  }
}
...
road.prototype.writeVehicleRoutes = function(umin, umax) {
  console.log("\nin road.writeVehicleRoutes: ID=", this.roadID, " length=",
    parseFloat(this.roadLen).toFixed(1), " nveh=", this.veh.length,
    " itime=", itime, "\n", "  duTactical=", this.duTactical,
    "  offrampIDs=", this.offrampIDs, "  lastExits=", this.offrampLastExits);
  var uminLoc = (typeof umin !== 'undefined') ? umin : 0;
  var umaxLoc = (typeof umax !== 'undefined') ? umax : this.roadLen;
  for (var i = 0; i < this.veh.length; i++) {
    var u = this.veh[i].u;
    if ((u > uminLoc) && (u < umaxLoc) && (this.veh[i].route.length > 0)) {
      console.log(" veh[", i, "].type=", this.veh[i].type, "  id=", this.veh[
          i].id, "  u=", parseFloat(this.veh[i].u, 10).toFixed(1), "  v=",
        parseFloat(this.veh[i].v, 10).toFixed(1), "  route=", this.veh[i]
        .route, "  divergeAhead=", this.veh[i].divergeAhead, "");
    }
  }
}
//######################################################################
...
road.prototype.writeDepotVehObjects = function(umin, umax) {
  console.log("itime=", itime, "in road.writeDepotVehObjects: roadID=", this
    .roadID, " nveh=", this.veh.length);
  var uminLoc = (typeof umin !== 'undefined') ? umin : 0;
  var umaxLoc = (typeof umax !== 'undefined') ? umax : this.roadLen;
  for (var i = 0; i < this.veh.length; i++) {
    if ((this.veh[i].isDepotObstacle()) || (this.veh[i].isTrafficLight())) {
      ...
      if ((u > uminLoc) && (u < umaxLoc)) {
        console.log("  veh[" + i + "].type=" + this.veh[i].type + "  id=" +
          this.veh[i].id + "  u=" + formd(this.veh[i].u) + "  v=" + formd(
            this.veh[i].v) + "");
      }
    }
  }
...
road.prototype.writeTrafficLights = function(umin, umax) {
  console.log("itime=", itime, " in road.writeTrafficLights:",
    " writing the road's operational TL objects", " roaroadID=", this
    .roadID, " nTL=", this.trafficLights.length);
  var uminLoc = (typeof umin !== 'undefined') ? umin : -1000;
  var umaxLoc = (typeof umax !== 'undefined') ? umax : this.roadLen;
  for (var i = 0; i < this.trafficLights.length; i++) {
    var u = this.trafficLights[i].u;
    if ((u > uminLoc) && (u < umaxLoc)) {
      console.log("  i=", i, "  id=" + this.trafficLights[i].id, "  u=" +
        formd(this.trafficLights[i].u), "  value=" + this.trafficLights[i]
        .value, "");
    }
  }
}
...
road.prototype.writeVehicleLongModels = function(umin, umax) {
  console.log("\nin road.writeVehicleLongModels(): ID=", this.roadID,
    " nveh=", this.veh.length, " itime=" + itime);
  var uminLoc = (typeof umin !== 'undefined') ? umin : 0;
  var umaxLoc = (typeof umax !== 'undefined') ? umax : this.roadLen;
  for (var i = 0; i < this.veh.length; i++) {
    if ((this.veh[i].u >= uminLoc) && (this.veh[i].u <= umaxLoc)) {
      console.log(" veh[" + i + "].type=" + this.veh[i].type + "  id=" + this
        .veh[i].id + "  u=" + parseFloat(this.veh[i].u, 10).toFixed(1) +
        "  v=" + parseFloat(this.veh[i].v, 10).toFixed(1) + "  speed=" +
        parseFloat(this.veh[i].speed, 10).toFixed(1) + "  v0=" + parseFloat(
          this.veh[i].longModel.v0).toFixed(1) + "  T=" + parseFloat(this
          .veh[i].longModel.T).toFixed(1) + "  a=" + parseFloat(this.veh[i]
          .longModel.a).toFixed(1) + "  acc=" + parseFloat(this.veh[i].acc)
        .toFixed(1) + "");
    }
  }
}
...
road.prototype.writeVehicleLCModels = function() {
  console.log("\nin road.writeVehicleLCModels(): ID=", this.roadID, " nveh=",
    this.veh.length, " itime=" + itime);
  for (var i = 0; i < this.veh.length; i++) {
    console.log(" veh[" + i + "].type=" + this.veh[i].type + " id=" + this
      .veh[i].id + " u=" + parseFloat(this.veh[i].u, 10).toFixed(1) +
      " v=" + parseFloat(this.veh[i].v, 10).toFixed(1) + " speed=" +
      parseFloat(this.veh[i].speed, 10).toFixed(1) +
      " LCmodel.bBiasRight=" + parseFloat(this.veh[i].LCModel.bBiasRight)
      .toFixed(1));
...
road.prototype.writeTrucksLC = function() {
  console.log("\nin road.writeTrucksLC(): nveh=", this.veh.length, " itime=" +
    itime);
  for (var i = 0; i < this.veh.length; i++) {
    if (this.veh[i].type === "truck") {
      console.log(" veh[" + i + "].type=" + this.veh[i].type + "  u=" +
        parseFloat(this.veh[i].u, 10).toFixed(1) + "  v=" + parseFloat(this
          .veh[i].v, 10).toFixed(1) + "  lane=" + this.veh[i].lane +
...
road.prototype.getNearestUof = function(otherRoad, u) {
  ...
  for (var i = 0; i <= otherRoad.nSegm; i++) {
    ...
    var dist2 = Math.pow(xOwn - otherRoad.xtab[i], 2) + Math.pow(yOwn -
      otherRoad.ytab[i], 2);
    if (false) {
      console.log("getNearestUof:", "  xOwn=", xOwn, " otherRoad.xtab=",
        otherRoad.xtab[i], "  yOwn=", yOwn, " otherRoad.ytab=", otherRoad
        .ytab[i], " dist2=", dist2, " dist2_min=", dist2_min, " uReturn=",
        uReturn);
    }
    if (dist2 < dist2_min) {
      dist2_min = dist2;
  ...
  if (false) {
    console.log("end road.getNearestUof: u=", u, " this.roadLen=", this
      .roadLen, " otherRoad.roadLen=", otherRoad.roadLen, " xOwn=", xOwn,
      " yOwn=", yOwn, " uReturn=", uReturn, " dist=", Math.sqrt(dist2));
  }
  return uReturn;
}
/**
...
road.prototype.findNearestVehTo = function(xUser, yUser, filterFun) {
  ...
  var iReturn = -1;
  for (var i = 0; i < this.veh.length; i++) {
    var filterPassed = (typeof filterFun === 'undefined') ? true : filterFun(
      this.veh[i]);
    ...
    if (filterPassed) {
      var u = this.veh[i].u;
      var dist2 = Math.pow(xUser - this.traj_x(u), 2) + Math.pow(yUser - this
        .traj_y(u), 2);
      if (dist2 < dist2_min) {
        success = true;
        dist2_min = dist2;
  ...
  if (false) {
    console.log("end road.getNearestVehTo:", " xUser=", xUser, " yUser=",
      yUser, " i=", i, " dist_min=", Math.sqrt(dist2_min));
  }
...
road.prototype.findNearestDistanceTo = function(xUser, yUser) {
  ...
  var sign_v = (Math.cos(phiNorm) * dxReturn + Math.sin(phiNorm) * dyReturn >
    0) ? 1 : -1;
  ...
  if (false) {
    console.log("end road.findNearestDistanceTo:", " roadID=", this.roadID,
      " xUser=", xUser, " yUser=", yUser, " dxReturn=", dxReturn,
      " dyReturn=", dyReturn, " dist=", dist, " uReturn=", uReturn,
      " phiNorm=", phiNorm, " vPhys=", vPhys, " vLanes=", vLanes);
  }
...
road.prototype.testCRG = function(xUser, yUser) {
  var distCrit = 0.8 * this.nLanes * this
  .laneWidth; // 0.5 => only inside road
  var dist2_min = 1e9;
  for (var i = 0; i <= this.nSegm; i++) {
    var dist2 = Math.pow(xUser - this.xtab[i], 2) + Math.pow(yUser - this
      .ytab[i], 2);
    if (dist2 < dist2_min) {
      dist2_min = dist2;
      this.iPivot = i;
  ...
  if (success) {
    console.log("road.testCRG: new CRG event initiated!", " dist_min=", Math
      .round(dist_min), " iPivot=", this.iPivot, " xPivot=", Math.round(this
        .xPivot), " yPivot=", Math.round(this.yPivot))
  }
  ...
  if (considerMergeRoad) {
    for (var i = imin; i <= imax; i++) {
      ...
      if ((u > uBegin - inflLen) && (u < uBegin + commonLen + inflLen)) {
        ...
        var approachFact = (u < uBegin) ? Math.pow(Math.sin(0.5 * Math.PI *
          urelBeg), 2) : (u > uBegin + commonLen) ? Math.pow(Math.sin(0.5 *
          Math.PI * urelEnd), 2) : 0;
        ...
        deltaX[i] = approachFact * deltaXold + (1 - approachFact) *
          deltaR_dot_e * e[0];
        deltaY[i] = approachFact * deltaYold + (1 - approachFact) *
          deltaR_dot_e * e[1];
      }
    }
  }
...
road.prototype.finishCRG = function() {
  ...
  chdUPoints[0] = 0;
  for (var i = 1; i <= this.nSegm; i++) {
    chdUPoints[i] = chdUPoints[i - 1] + Math.sqrt(Math.pow(this.xtab[i] - this
      .xtab[i - 1], 2) + Math.pow(this.ytab[i] - this.ytab[i - 1], 2));
  }
  ...
  for (var inew = 1; inew < this.nSegm; inew++) {
    ...
    while (chdUPoints[iUpper] < unew) {
      iUpper++;
    }
    var du = chdUPoints[iUpper] - chdUPoints[iUpper - 1];
    var rest = (unew - chdUPoints[iUpper - 1]) / du; // in [0,1]
    xtabNew[inew] = (1 - rest) * this.xtab[iUpper - 1] + rest * this.xtab[
      iUpper];
    ytabNew[inew] = (1 - rest) * this.ytab[iUpper - 1] + rest * this.ytab[
      iUpper];
    if (false) {
      var drnew = Math.sqrt(Math.pow(xtabNew[inew] - xtabNew[inew - 1], 2) +
        Math.pow(ytabNew[inew] - ytabNew[inew - 1], 2));
      console.log("inew=", inew, " iUpper=", iUpper, " rest=", rest,
        " xtabNew[inew]=", xtabNew[inew], " ytabNew[inew]=", ytabNew[inew],
  ...
  if (false) {
    ...
    for (var i = 0; i <= this.nSegm; i++) {
      console.log("i=", i, " xytabOld=", Math.round(this.xtabOld[i]), "", Math
        .round(this.ytabOld[i]), " xytab=", Math.round(this.xtab[i]), "",
        Math.round(this.ytab[i]), " xytabNew=", Math.round(xtabNew[i]), "",
        Math.round(ytabNew[i]), " du=", parseFloat(chdUPoints[i] -
          chdUPoints[Math.max(i - 1, 0)]).toFixed(2), " duNew=", parseFloat(
          this.roadLen / this.nSegm).toFixed(2));
    }
  }
...
road.prototype.smoothLocally = function(iCenter, iWidth) {
  ...
  for (var i = iCenter - iw; i <= iCenter + iw; i++) {
    ...
    if (false) {
      console.log("smoothLocally: i=", i, " xtabNew[i]=", xtabNew[i],
  ...
  for (var i = iCenter - iw; i <= iCenter + iw; i++) {
    if (isNaN(xtabNew[i]) || isNaN(ytabNew[i])) {
      console.log("road.SmoothLocally: i=", i, " iCenter=", iCenter, "iw=",
        iw, ": xtabNew[i] or ytabNew[i] are NaN's!" + "\ndoing nothing");
      return;
    }
  }
  // copy to this.xytab
...
road.prototype.smooth = function() {
  ...
  if (false) {
    ...
    for (var i = 0; i <= n; i++) {
      console.log("i=", i, " xtabOld=", this.xtab[i], " xtabNew=", xtabNew[
      i]);
    }
  }
  for (var i = 0; i < n; i++) {
...
*/
road.prototype.initializeMicro = function(types, lengths, widths, longPos,
  lanesReal, speeds, iEgo) {
  var nvehInit = types.length;
  if ((lengths.length != nvehInit) || (widths.length != nvehInit) || (longPos
      .length != nvehInit) || (lanesReal.length != nvehInit) || (speeds
      .length != nvehInit)) {
  ...
  for (var i = 0; i < types.length; i++) {
    ...
    var type = (types[i] === 0) ? "car" : (types[i] === 1) ? "truck" :
      "obstacle";
    var lane = Math.round(lanesReal[i]);
    var vehNew = new vehicle(lengths[i], widths[i], longPos[i], lane, speeds[
      i], type);
  ...
  // check
  if (false) {
    console.log("road.initializeMicro: initialized with ", this.veh.length,
      " vehicles");
...
road.prototype.updateTruckFrac = function(fracTruck, mismatchTolerated) {
  if (this.veh.length > 0) {
    ...
    this
  .updateEnvironment(); //needs veh[i].iLag etc, so actual environment needed
    var nActual = 0;
    var nTruck = 0;
    for (var i = 0; i < this.veh.length; i++) {
    ...
    var fracTruckReal = nTruck /
    nActual; // int division gen. results in double: OK!
    ...
    if (Math.abs(fracTruckReal - fracTruck) > mismatchTolerated) {
      ...
      var diffSpace = ((fracTruckTooLow) ? -1 : 1) * (truck_length -
        car_length);
      ...
      if (
        fracTruckTooLow) { // change cars->trucks on the right lane if possible
        var maxSpace = 0;
        for (var lane = this.nLanes - 1; lane >= 0; lane--) {
          if (!success) {
            for (var i = 0; i < nActual; i++) {
              if ((this.veh[i].lane === lane) && (this.veh[i].type ===
                  candidateType)) {
...
//#####################################################
road.prototype.setOfframpInfo = function(offrampIDs, offrampLastExits,
  offrampToRight) {
  ...
  console.log("\nin road.setOfframpInfo: ID=", this.roadID, "  offrampIDs=",
    offrampIDs, "  this.offrampIDs=", this.offrampIDs);
}
//#####################################################
...
road.prototype.getNextOffIndex = function(u) {
  ...
  for (var iOff = 0; iOff < this.offrampIDs.length; iOff++) {
    ...
    if ((this.isRing) && (uExit < u)) {
      du = uExit - u + this.roadLen;
    }
    if (du < duNearest) {
      index = iOff;
      duNearest = du;
...
//#####################################################
road.prototype.setCFModelsInRange = function(umin, umax, longModelCar,
  longModelTruck) {
  for (var i = 0; i < this.veh.length; i++) {
    var u = this.veh[i].u;
    if ((u > umin) && (u < umax)) {
      if (deepCopying) {
        ...
        if (this.veh[i].type === "car") {
          this.veh[i].longModel = longModelCar;
        }
        if (this.veh[i].type === "truck") {
          this.veh[i].longModel = longModelTruck;
        }
      }
    }
  }
...
//#####################################################
road.prototype.setLCModelsInRange = function(umin, umax, LCModelCar,
  LCModelTruck) {
  ...
  for (var i = 0; i < this.veh.length; i++) {
    ...
    if ((u > umin) && (u < umax)) {
      if (this.veh[i].type === "car") {
        this.veh[i].LCModel = LCModelCar;
      }
      if (this.veh[i].type === "truck") {
        this.veh[i].LCModel = LCModelTruck;
  ...
  for (var i = 0; i < this.veh.length; i++)
    if (this.veh[i].isRegularVeh()) {
      ...
      if ((u > umin) && (u < umax)) {
        ...
        this.veh[i].LCModel = (toRight) ? this.LCModelMandatoryRight : this
...
road.prototype.update_iLeadRight = function(i) {
  ...
  if (this.veh[i].lane < this.nLanes - 1) {
    ...
    success = ((i === iLeadRight) || (this.veh[iLeadRight].lane === this.veh[
      i].lane + 1));
    while (!success) {
      iLeadRight = (iLeadRight === 0) ? n - 1 : iLeadRight - 1;
      success = ((i === iLeadRight) || (this.veh[iLeadRight].lane === this
        .veh[i].lane + 1));
    }
  } else {
    iLeadRight = -10;
...
road.prototype.update_iLagRight = function(i) {
  ...
  if (this.veh[i].lane < this.nLanes - 1) {
    ...
    success = ((i === iLagRight) || (this.veh[iLagRight].lane === this.veh[i]
      .lane + 1));
    while (!success) {
      iLagRight = (iLagRight === n - 1) ? 0 : iLagRight + 1;
      success = ((i === iLagRight) || (this.veh[iLagRight].lane === this.veh[
        i].lane + 1));
    }
  } else {
    iLagRight = -10;
...
road.prototype.update_iLeadLeft = function(i) {
  ...
  if (this.veh[i].lane > 0) {
    ...
    success = ((i === iLeadLeft) || (this.veh[iLeadLeft].lane === this.veh[i]
      .lane - 1));
    while (!success) {
      iLeadLeft = (iLeadLeft === 0) ? n - 1 : iLeadLeft - 1;
      success = ((i === iLeadLeft) || (this.veh[iLeadLeft].lane === this.veh[
        i].lane - 1));
    }
  } else {
    iLeadLeft = -10;
...
road.prototype.update_iLagLeft = function(i) {
  ...
  if (this.veh[i].lane > 0) {
    ...
    success = ((i === iLagLeft) || (this.veh[iLagLeft].lane === this.veh[i]
      .lane - 1));
    while (!success) {
...
road.prototype.calcAccelerations = function() {
  ...
  for (var i = 0; i < this.veh.length; i++) {
    ...
    if (iLead === -100) {
      console.log("road.calcAccelerations: i=", i, " iLead=", iLead,
    ...
    if (this.veh[i].id > 1) {
      this.veh[i].acc = (this.veh[i].isRegularVeh()) ? this.veh[i].longModel
        .calcAcc(s, speed, speedLead, accLead) : 0;
    }
    ...
    if (this.veh[i].id === 1) {
      if (false) {
        console.log("in road: ego vehicle: u=", this.veh[i].u, " v=", this
          .veh[i].v, " speed_u=", this.veh[i].speed, "  acc_u=", this.veh[i]
          .acc);
      }
    }
    if (false) {
      ...
      console.log("after calcAccelerations: i=" + i + " iLead=" + iLead +
        " pos=" + this.veh[i].u + " lane=" + this.veh[i].v + " s=" + s +
        " speed=" + speed + " v0=" + this.veh[i].longModel.v0 +
        " speedLead=" + speedLead + " acc=" + this.veh[i].acc);
    }
  }
}
//#######################################################################
...
road.prototype.updateEgoVeh = function(externalEgoVeh) {
  ...
  var acc_v = (externalEgoVeh.aLat + roadCurv * ego.speed * ego.speed) / this
    .laneWidth; // [lanes/s^2]
  ...
  if (externalEgoVeh.latCtrlModel === 2) {
    ...
    ego.v = (this.nLanes - 1) + (externalEgoVeh.v - xRoadAxis) / this
      .laneWidth;
    ego.dvdt = (externalEgoVeh.vv - dotxRoadAxis) / this.laneWidth;
    //ego.dvdt=(externalEgoVeh.vv-0)/this.laneWidth;
    if (false) {
      console.log("road.updateEgoVeh: latCtrlModel=", externalEgoVeh
        .latCtrlModel, "ego.v=", ego.v, " ego.dvdt=", ego.dvdt);
    }
  }
  ego.lane = Math.round(ego.v);
  if (false) {
    console.log("in road.updateEgoVeh:", " time=", parseFloat(time).toFixed(
      2), " dt=", parseFloat(dt).toFixed(2), " laneWidth=", parseFloat(this
        .laneWidth).toFixed(1), " acc=", parseFloat(ego.acc).toFixed(2),
      " acc_v[m/s^2]=", parseFloat(acc_v).toFixed(2), "\n         u=",
      parseFloat(ego.u).toFixed(2), " v[laneWidth]=", parseFloat(ego.v)
      .toFixed(2), " lane=", ego.lane, "\n         speedLong=", parseFloat(
        ego.speed).toFixed(2), " vv=", parseFloat(vv).toFixed(2), " dvdt=",
...
road.prototype.updateSpeedPositions = function() {
  ...
  for (var i = 0; i < this.veh.length; i++) {
    if ((this.veh[i].id != 1) && (this.veh[i].isRegularVeh())) {
      ...
      this.veh[i].u += Math.max(0, this.veh[i].speed * dt + 0.5 * this.veh[i]
        .acc * dt * dt);
      // longitudinal speed update
      this.veh[i].speed = Math.max(this.veh[i].speed + this.veh[i].acc * dt,
        0);
      // MT 2019-09: quick hack since left vehicles always a bit
      // faster than right ones: "push on right
      if ((this.veh[i].lane == this.nLanes - 1) && (this.veh[i].speed >
        1.5)) {
...
road.prototype.changeLanes = function() {
  if (false) { //!!
    this.writeVehicles();
    var testLongTruck = new ACC(IDMtruck_v0, IDMtruck_T, IDM_s0, IDMtruck_a,
      IDM_b);
    var testLongTruckIDM = new IDM(IDMtruck_v0, IDMtruck_T, IDM_s0,
      IDMtruck_a, IDM_b);
    s = 85.7;
    speed = 18.7;
    speedLead = 15.7;
...
road.prototype.doChangesInDirection = function(toRight) {
  var log = false;
  if (log && toRight) {
    console.log("changeLanes: before changes to the right");
  }
  if (log && (!toRight)) {
    console.log("changeLanes: before changes to the left");
  }
  ...
  for (var i = 0; i < this.veh.length; i++)
    if ((this.veh[i].u > this.uminLC) && (this.veh[i].isRegularVeh())) {
      ...
      var lastChangeSufficTimeAgo = (this.veh[i].dt_afterLC > this
        .waitTime) && (this.veh[i].dt_lastPassiveLC > 0.2 * this.waitTime);
      if (false) {
        //if(itime==100){
      ...
      if (targetLaneExists && lastChangeSufficTimeAgo) {
        ...
        var iLeadNew = (toRight) ? this.veh[i].iLeadRight : this.veh[i]
          .iLeadLeft;
        var iLagNew = (toRight) ? this.veh[i].iLagRight : this.veh[i]
        .iLagLeft;;
        ...
        if ((this.veh[i].id != 1) // not an ego-vehicle
          ...
          && (this.veh[iLeadNew].dt_afterLC > this
          .waitTime) // lower time limit
          && (this.veh[iLagNew].dt_afterLC > this.waitTime)
          ) { // for serial LC
          //console.log("changeLanes: i=",i," cond 2 passed");
          var acc = this.veh[i].acc;
          var accLead = this.veh[iLead].acc;
          var accLeadNew = this.veh[iLeadNew]
          .acc; // leaders: exogen. for MOBIL
          var speed = this.veh[i].speed;
          var speedLeadNew = this.veh[iLeadNew].speed;
          var sNew = this.veh[iLeadNew].u - this.veh[iLeadNew].length - this
            .veh[i].u;
          var sLagNew = this.veh[i].u - this.veh[i].length - this.veh[iLagNew]
            .u;
          ...
          var accNew = this.veh[i].longModel.calcAcc(sNew, speed,
            speedLeadNew, accLeadNew);
          ...
          if (this.veh[iLagNew].longModel === undefined) {
            console.log("vehicle ", iLagNew, " has no longModel! vehicle=",
              this.veh[iLagNew]);
            this.writeNonregularVehicles(0, this.roadLen);
          }
          var accLagNew = this.veh[iLagNew].longModel.calcAcc(sLagNew,
            speedLagNew, speed, accNew);
          ...
          var log = false;
          //var log=true;
          var MOBILOK = this.veh[i].LCModel.realizeLaneChange(vrel, acc,
            accNew, accLagNew, toRight, log);
          // only test output
          if (MOBILOK && (this.veh[i].id === 107)) { //!!
            var s = this.veh[iLead].u - this.veh[iLead].length - this.veh[i]
            .u;
            var accLead = this.veh[iLead].acc;
            var speed = this.veh[i].speed;
            var speedLead = this.veh[iLead].speed;
            var accCalc = this.veh[i].longModel.calcAcc(s, speed, speedLead,
              accLead);
            console.log("MOBILOK!change successfully initiated!",
              "\n  vehicle id", this.veh[i].id, " type ", this.veh[i].type,
              " from lane ", this.veh[i].lane, " to lane", newLane,
              "\n  u=", parseFloat(this.veh[i].u).toFixed(1), " s=",
              parseFloat(s).toFixed(1), " speed=", parseFloat(speed)
              .toFixed(1), " speedLead=", parseFloat(speedLead).toFixed(1),
              " accLead=", parseFloat(accLead).toFixed(1), " acc=",
              parseFloat(this.veh[i].acc).toFixed(1), " accNew=",
              parseFloat(accNew).toFixed(1),
              //" accCalc=",parseFloat(accCalc).toFixed(1),
              "\n  longModel=", this.veh[i].longModel, "\n  veh[iLead]=",
              this.veh[iLead], "");
            this.writeVehicles();
          }
          changeSuccessful = (this.veh[i].isRegularVeh()) && (sNew > 0) && (
            sLagNew > 0) && MOBILOK;
          //!!! MT 2019-09: prevent trucks to change to the left by force
          if ((scenarioString == "OnRamp_BaWue") || (scenarioString ==
              "roadworks_BaWue")) {
            if (changeSuccessful && (this.veh[i].type === "truck") && (!
                toRight)) {
              console.log(
...
*/
road.prototype.mergeDiverge = function(newRoad, offset, uBegin, uEnd, isMerge,
  toRight, ignoreRoute, prioOther, prioOwn) {
  var log = false;
  //var log=(this.roadID==7)&&isMerge;
  //var log=((this.roadID===10)&&(this.veh.length>0)&&(!isMerge));
  var padding = this
  .padding; // visib. extension for orig drivers to target vehs
  var paddingLTC = // visib. extension for target drivers to orig vehs
    (isMerge && prioOwn) ? this.paddingLTC : 0;
  var loc_ignoreRoute = (typeof ignoreRoute === 'undefined') ? false :
    ignoreRoute; // default: routes  matter at diverges
  ...
  var originVehicles = this.getTargetNeighbourhood(uBegin - paddingLTC, uEnd,
    originLane); // padding only for LT coupling!
  var targetVehicles = newRoad.getTargetNeighbourhood(uNewBegin - padding,
    uNewEnd + padding, targetLane);
  ...
  if (log) {
    console.log("\n\nin road.mergeDiverge: itime=", itime, " ID=", this
      .roadID, " targetVehicles.length=", targetVehicles.length,
  ...
  var success = ((targetVehicles.length === 0) && (originVehicles.length >
    0) && originVehicles[0].isRegularVeh() && (originVehicles[0].u >=
      uBegin) // otherwise only LT coupl
    && (loc_ignoreRoute || originVehicles[0].divergeAhead));
  if (log) {
    console.log(" road.mergeDiverge: 2a:  success=", success);
  }
  if (success) {
    iMerge = 0;
  ...
  // (2b) otherwise select the first suitable candidate of originVehicles
    ...
    var duFollower = -1000;
    var leaderNew = new vehicle(0, 0, uNewBegin + 10000, targetLane, 0,
    "car");
    var followerNew = new vehicle(0, 0, uNewBegin - 10000, targetLane, 0,
      "car");
    if (log) {
    ...
    for (var i = 0;
      (i < originVehicles.length) && (!success); i++) {
      if (log) {
        console.log(" i=", i, " isRegularVeh=", originVehicles[i]
          .isRegularVeh(), " loc_ignoreRoute=", loc_ignoreRoute,
          " originVehicles[i].divergeAhead=", originVehicles[i].divergeAhead
          );
      }
      if (originVehicles[i].isRegularVeh() && (loc_ignoreRoute ||
        ...
        for (var j = 0; j < targetVehicles.length; j++) {
          ...
          if (log) {
            console.log(" i=", i, " j=", j, " jTarget=", jTarget, " du=", du,
              " duLeader=", duLeader, " duFollower=", duFollower);
          }
        }
        ...
        var LCModel = (toRight) ? this.LCModelMandatoryRight : this
          .LCModelMandatoryLeft;
        var vrel = originVehicles[i].speed / originVehicles[i].longModel.v0;
        var acc = originVehicles[i].acc;
        var accNew = originVehicles[i].longModel.calcAcc(sNew, speed,
          speedLeadNew, accLeadNew);
        var accLag = followerNew.acc;
        var accLagNew = originVehicles[i].longModel.calcAcc(sLagNew,
          speedLagNew, speed, accNew);
        // MOBIL decisions
        var prio_OK = (!loc_prioOther) || loc_prioOwn || (!LCModel
          .respectPriority(accLag, accLagNew));
        var MOBILOK = LCModel.realizeLaneChange(vrel, acc, accNew, accLagNew,
          toRight, false);
        success = prio_OK && inChangeRegion && MOBILOK && (originVehicles[i]
          .isRegularVeh()) && (sNew > 0) && (sLagNew > 0);
        ...
        if (success && log) {
          //if(true){
          console.log("testing origin veh " + i + " type=" + originVehicles[i]
            .type + " uTarget=" + uTarget);
          ...
          console.log("  acc=" + acc + " accNew=" + accNew + " accLagNew=" +
            accLagNew);
          console.log("  duLeader=" + duLeader + "  duFollower=" +
            duFollower + " sLagNew=" + sLagNew + " MOBILOK=" + MOBILOK +
            " success=" + success);
        }
      } // !obstacle
    } // merging veh loop
  ...
  if (isMerge && loc_prioOwn) {
    ...
    // use it
    for (var j = 0; j < targetVehicles.length; j++) {
      var sStop = stopLinePosNew - targetVehicles[j]
      .u; // gap to stop for target veh
      var speedTarget = targetVehicles[j].speed;
      var accTargetStop = targetVehicles[j].longModel.calcAcc(sStop,
        speedTarget, 0, 0);
      var allOrigVehsAreLeaders = true;
      var iLast = -1;
      for (var i = originVehicles.length - 1;
        (i >= 0) && (iLast == -1); i--) {
      ...
      if ((iLast > -1) && targetVehicles[j].isRegularVeh()) {
        ...
        if (lastOrigIsLeader) {
          ...
          var accLTC = targetVehicles[j].longModel.calcAcc(s, speedTarget,
            speedOrig, 0);
          var accTarget = Math.min(targetVehicles[j].acc, Math.max(accLTC,
            accTargetStop));
          if (accTarget > -bSafe) {
        ...
        //if(this.roadID==7){
        if (false) {
          console.log("target id=", targetVehicles[j].id, " iLast id=",
            originVehicles[iLast].id, " lastOrigIsLeader=",
            lastOrigIsLeader, " sStop=", parseFloat(sStop).toFixed(1),
  ...
  if (success) { // do the actual merging
    ...
    if (false) {
      ...
      console.log("Actual merging: merging origin vehicle " + iOrig +
        " of type " + this.veh[iOrig].type + " from origin position " + this
        .veh[iOrig].u + " and origin lane" + originLane +
        " to target position " + uTarget + " and target lane" + targetLane);
      console.log(" this.veh[iOrig].divergeAhead)=" + this.veh[iOrig]
        .divergeAhead);
    }
    var changingVeh = this.veh[iOrig]; //originVehicles[iMerge];
    var vOld = (toRight) ? targetLane - 1 : targetLane +
    1; // rel. to NEW road
  ...
  // too few vehicles, generate one per time step in largest gap
    ...
    if (success) { // otherwise, no veh added
      if (!emptyLanes) {
        ...
        speedNew = 0.5 * (this.veh[k].speed + this.veh[this.veh[k].iLead]
          .speed);
      }
      var vehNew = new vehicle(vehLength, vehWidth, uNew, laneNew, speedNew,
        vehType); //updateDensity
      if (emptyLanes) {
        vehNew.speed = longModelTruck.v0;
      }
      this.veh.splice(k, 0,
      vehNew); // add vehicle at position k  (k=0 ... n-1)
    }
  }
...
road.prototype.updateBCup = function(Qin, dt, route) {
  ...
  this.route = (typeof route === 'undefined') ? [0] :
  route; // handle opt. args
  ...
  if (this.inVehBuffer >= 1) {
    ...
    if ((!success) && ((!this.setTrucksAlwaysRight) || (vehType == "car"))) {
      ...
      for (var candLane = this.nLanes - 1; candLane >= 0; candLane--) {
        ...
        space = (iLead >= 0) ? this.veh[iLead].u - this.veh[iLead].length :
    ...
    if (success) {
      ...
      var speedNew = Math.min(v0New, longModelNew.speedlimit, space /
        longModelNew.T);
      var vehNew = new vehicle(vehLength, vehWidth, uNew, lane, speedNew,
        vehType);
      if (deepCopying) {
      ...
      if (false) {
        console.log("road.updateBCup: ID=", this.roadID,
          " new vehicle at pos u=0, lane=", lane, " type=", vehType, " s=",
          space, " speed=", speedNew);
        console.log(this.veh.length);
        if (false) {
          for (var i = 0; i < this.veh.length; i++) {
            console.log("i=", i, " u=", this.veh[i].u, " route=", this.veh[i]
              .route, " longModel=", this.veh[i].longModel);
          }
        }
      }
    }
...
road.prototype.getTargetNeighbourhood = function(umin, umax, targetLane) {
  ...
  for (var i = 0; i < this.veh.length; i++) {
    ...
    if ((this.veh[i].lane === targetLane) && (this.veh[i].u >= umin) && (this
        .veh[i].u <= umax)) {
      if (firstTime === true) {
        this.iTargetFirst = i;
        firstTime = false;
  ...
  if (false) {
    console.log("in road.getTargetNeighbourhood(umin=" + umin + ", umax=" +
      umax + ", targetLane=" + targetLane + ")");
    for (iTarget = 0; iTarget < targetVehicles.length; iTarget++) {
      console.log("targetVehicles[" + iTarget + "].u=" + targetVehicles[
        iTarget].u);
    }
  }
...
//####################################################
*/
road.prototype.updateModelsOfAllVehicles = function(longModelCar,
  longModelTruck, LCModelCar, LCModelTruck, LCModelMandatory) {
  ...
  this.longModelTacticalCar = new ACC(longModelCar.v0, longModelCar.T,
    longModelCar.s0, longModelCar.a, longModelCar.b);
  this.longModelTacticalTruck = new ACC(longModelTruck.v0, longModelTruck.T,
    longModelTruck.s0, longModelTruck.a, longModelTruck.b);
  this.LCModelMandatoryRight = new MOBIL(LCModelMandatory.bSafe,
    LCModelMandatory.bSafeMax, LCModelMandatory.p, LCModelMandatory.bThr,
    LCModelMandatory.bBiasRight);
  this.LCModelMandatoryLeft = new MOBIL(LCModelMandatory.bSafe,
    LCModelMandatory.bSafeMax, LCModelMandatory.p, LCModelMandatory.bThr, -
    LCModelMandatory.bBiasRight);
  //console.log("road.LCModelMandatoryLeft=",this.LCModelMandatoryLeft);
  this.LCModelTacticalRight = new MOBIL(LCModelMandatory.bSafe,
    LCModelMandatory.bSafeMax, LCModelMandatory.p, LCModelMandatory.bThr,
    LCModelMandatory.bBiasRight);
  this.LCModelTacticalLeft = new MOBIL(LCModelMandatory.bSafe,
    LCModelMandatory.bSafeMax, LCModelMandatory.p, LCModelMandatory.bThr, -
    LCModelMandatory.bBiasRight);
  // normal acc and LC:
  ...
  if (deepCopying) {
    for (var i = 0; i < this.veh.length; i++) {
      if (this.veh[i].isRegularVeh()) { // then do nothing
        ...
        this.veh[i].LCModel = (this.veh[i].type === "car") ? LCModelCar :
          LCModelTruck;
      }
      if (false) {
    ...
    for (var i = 0; i < this.veh.length; i++) {
      if (this.veh[i].isRegularVeh()) { // then do nothing
        this.veh[i].longModel = (this.veh[i].type === "car") ? longModelCar :
          longModelTruck;
        this.veh[i].LCModel = (this.veh[i].type === "car") ? LCModelCar :
          LCModelTruck;
      }
  ...
  if (this.duTactical > 0)
    for (var i = 0; i < this.veh.length; i++) {
      if (this.veh[i].isRegularVeh()) {
        ...
        var nextOfframpNearby = (this.veh[i].isRegularVeh()) && (iNextOff > -
          1) && (uLastExit - this.veh[i].u < this.duTactical);
        if (nextOfframpNearby) {
          if (false) {
            console.log("in road.updateModels... iveh=" + i + " iNextOff=" +
              iNextOff + " u=" + this.veh[i].u + " uLastExit=" + uLastExit);
          }
          ...
          if (tacticalLC) {
            ...
            if (deepCopying) {
              thisVeh.longModel.alpha_v0 = Math.max(0.1, 0.5 * duRemaining /
                this.duTactical);
            } else {
              thisVeh.longModel = (thisVeh.type === "truck") ? this
                .longModelTacticalTruck : this.longModelTacticalCar;
            }
            thisVeh.LCModel = (toRight) ? this.LCModelTacticalRight : this
              .LCModelTacticalLeft;
            if (false) {
              console.log(
                "road.updateModelsOfAllVehicles: apply tacticalLC to Veh " +
                i + "!" + " id=" + thisVeh.id + " route=" + thisVeh.route +
                " offID=" + offID + " u=" + parseFloat(thisVeh.u).toFixed(
                1) + " uLastExit=" + parseFloat(uLastExit).toFixed(1) +
...
road.prototype.get_phi = function(u) {
  ...
  if ((Math.abs(dx) < smallVal) && (Math.abs(dy) < smallVal)) {
    console.log("road.get_phi: id=", this.roadID, " uLoc+du=", uLoc + du,
      " uLoc-du=", uLoc - du, " this.traj_x(uLoc+du)=", this.traj_x(uLoc +
        du), " this.traj_x(uLoc-du)=", this.traj_x(uLoc - du),
...
*/
road.prototype.draw = function(roadImg1, roadImg2, scale, changedGeometry, umin,
  umax, movingObs, uObs, xObs, yObs) {
  ...
  var factor = 1 + this.nLanes * this.laneWidth * this
  .draw_curvMax; // " stitch factor"
  ...
  if (changedGeometry) {
    ...
    for (var iSegm = 0; iSegm < this.nSegm; iSegm++) {
      ...
      if (false) {
        console.log("road.draw: iSegm=" + iSegm + " u=" + u + " xPhys=" + this
          .draw_x[iSegm] + " yPhys=" + this.draw_y[iSegm] + " phi=" + this
          .draw_phi[iSegm]);
      }
    }
  }
  ...
  var nSegmLine = 2 * Math.round(0.5 * duLine / (this.roadLen / this
  .nSegm)); // 0,2,4...
  ...
  for (var iSegm = 0; iSegm < this.nSegm; iSegm++) {
    ...
    if (filterPassed) {
      ...
      var wSegmPix = scale * (this.nLanes * this.laneWidth +
        boundaryStripWidth);
      var xCenterPix = scale * (this.draw_x[iSegm] - this.traj_x(uRef) +
      xRef);
      var yCenterPix = -scale * (this.draw_y[iSegm] - this.traj_y(uRef) +
        yRef);
      ctx.setTransform(cosphi, -sinphi, +sinphi, cosphi, xCenterPix,
        yCenterPix);
      var roadImg = (iSegm % nSegmLine < nSegmLine / 2) ? roadImg1 : roadImg2;
      ctx.drawImage(roadImg, -0.5 * lSegmPix, -0.5 * wSegmPix, lSegmPix,
        wSegmPix);
      if (false) {
        ...
        console.log("road.draw: ID=", this.roadID, " iSegm=", iSegm,
          ...
          " this.traj_y(this.roadLen-50)=", formd(this.traj_y(this.roadLen -
            50)),
...
road.prototype.drawVehicles = function(carImg, truckImg, obstacleImg, scale,
  speedmin, speedmax, umin, umax, movingObs, uObs, xObs, yObs) {
  var noRestriction = (typeof umin === 'undefined');
  var movingObserver = (typeof movingObs === 'undefined') ? false : movingObs;
  var uRef = (movingObserver) ? uObs : 0;
  var xRef = (movingObserver) ? xObs : this.traj_x(0);
  ...
  for (var i = 0; i < this.veh.length; i++) {
    ...
    if (filterPassed) {
      this.drawVehicle(i, carImg, truckImg, obstacleImg, scale, speedmin,
        speedmax, xOffset, yOffset, this, 0);
    }
  }
}
//######################################################################
...
*/
road.prototype.drawVehiclesGenTraj = function(carImg, truckImg, obstacleImg,
  scale, speedmin, speedmax, umin, umax, otherRoad, uOffset) {
  if (this.doGridding) {
    console.log(
      "Error: cannot use road.drawVehiclesGenTraj with active gridding",
      " (user-can change road geometry): ");
  ...
  for (var i = 0; i < this.veh.length; i++) {
    // do not draw vehicles outside limits
    //  or if it is a virtual traffic-light vehicle (if TL is red)
    var filterPassed = (!this.veh[i].isTrafficLight()) && ((this.veh[i].u >=
      umin) && (this.veh[i].u <= umax));
    if (filterPassed) {
      this.drawVehicle(i, carImg, truckImg, obstacleImg, scale, speedmin,
        speedmax, xOffset, yOffset, otherRoad, uOffset);
    }
  }
}
//###############################################################
...
road.prototype.drawVehicle = function(i, carImg, truckImg, obstacleImg, scale,
  ...
  var vCenterPhys = this.laneWidth * (this.veh[i].v - 0.5 * (this.nLanes -
  1));
  // (3) determine vehicle center xCenterPix, yCenterPix
  // in pixel coordinates
  var phiRoad = (useOtherTraj) ? otherRoad.get_phi(uCenterPhys + du) : this
    .get_phi(uCenterPhys);
  var phiVehRel = (this.veh[i].speed < 0.1) ? 0 : -Math.atan(this.veh[i]
    .dvdt * this.laneWidth / this.veh[i].speed);
  ...
  var obstacleImgIndex = (this.veh[i].isSpecialVeh()) ? (this.veh[i].id -
    49) % obstacleImgs.length : 0;
  if (type === "obstacle") {
  ...
  var xCenterPix = scale * (this.traj_x(uCenterPhys + du) + vCenterPhys *
    sphiRoad - xOffset); // -xOffset=-this.traj_x(uRef)+xRef)
  var yCenterPix = -scale * (this.traj_y(uCenterPhys + du) - vCenterPhys *
    cphiRoad - yOffset);
  if (useOtherTraj) {
    xCenterPix = scale * (otherRoad.traj_x(uCenterPhys + du) + vCenterPhys *
      sphiRoad - xOffset); // -xOffset=-this.traj_x(uRef)+xRef)
    yCenterPix = -scale * (otherRoad.traj_y(uCenterPhys + du) - vCenterPhys *
      cphiRoad - yOffset);
  }
  ...
  vehImg = (type === "car") ? carImg : (type === "truck") ? truckImg :
    obstacleImg;
  ctx.setTransform(cphiVeh, -sphiVeh, +sphiVeh, cphiVeh, xCenterPix,
    yCenterPix);
  ctx.drawImage(vehImg, -0.5 * vehLenPix, -0.5 * vehWidthPix, vehLenPix,
    vehWidthPix);
  ...
  if (type != "obstacle") {
    ...
    ctx.fillStyle = (this.veh[i].colorStyle == 0) ? colormapSpeed(speed,
        speedmin, speedmax, type, isEgo, time) : (this.veh[i].colorStyle ==
      1) ? "rgba(0,0,255,0.5)" : (this.veh[i].colorStyle == 2) ?
    ...
    if (isEgo || this.veh[i].isPerturbed() || (this.veh[i].colorStyle > 0)) {
      ...
      ctx.strokeRect(-0.50 * effLenPix, -0.50 * effWPix, 1.0 * effLenPix,
        1.0 * effWPix);
      ctx.strokeRect(-0.55 * effLenPix, -0.55 * effWPix, 1.1 * effLenPix,
        1.1 * effWPix);
      ctx.strokeRect(-0.60 * effLenPix, -0.60 * effWPix, 1.2 * effLenPix,
        1.2 * effWPix);
    }
  }
  ...
  if (false) {
    //if(this.veh[i].v>2){
    console.log("in road.drawVehicles: itime=", itime, +" u=" + this.veh[i]
      .u + " v=" + this.veh[i].v + " xCenterPix=" + xCenterPix +
...
road.prototype.updateSpeedlimits = function(trafficObjects) {
  ...
  for (var i = 0; i < trafficObjects.trafficObj.length; i++) {
    ...
    if ((obj.type === 'speedLimit') && (obj.isActive) && (obj.road.roadID ==
        this.roadID)) {
      success = true;
      var speedL = obj.value / 3.6; // in m/s
      if (false) {
        console.log("road.updateSpeedlimits: speed limit ", formd(speedL),
          " starting at ", formd(obj.u));
      }
      while ((iveh < this.veh.length) && (this.veh[iveh].u > obj.u -
        duAntic)) {
        var targetVeh = this.veh[iveh];
        if (targetVeh.isRegularVeh()) {
          targetVeh.longModel.speedlimit = (targetVeh.type === "truck") ? Math
            .min(speedL, speedL_truck) : speedL;
        }
        if (false) {
          console.log("iveh=", iveh, " u=", formd(targetVeh.u), " obj.u=",
            formd(obj.u), " isRegVeh=", targetVeh.isRegularVeh(),
            " speedlimit_kmh=", formd0(3.6 * targetVeh.longModel.speedlimit)
            );
        }
        iveh++;
      }
      if (iveh == this.veh.length) {
        return;
      } // otherwise risk of range excess
    }
  }
  if (!success) {
...
road.prototype.dropObject = function(trafficObj) {
  ...
  console.log("itime=", itime, " in road.dropObject: trafficObj.u=", u,
  ...
  if (trafficObj.type === 'obstacle') {
    var roadVehicle = new vehicle(trafficObj.len, trafficObj.width, u, lane,
      0, "obstacle"); //=trafficObj.type
    ...
    console.log("  end road.dropObject: dropped obstacle at uDrop=", u,
      " lane=", lane, " id=", roadVehicle.id, " imgNumber=", roadVehicle
      .imgNumber);
  }
...
road.prototype.addTrafficLight = function(depotObject) {
  ...
  if (true) {
    console.log("itime=", itime, " road.addTrafficLight: roadID=", this
      .roadID, " added traffic light id=", depotObject.id, " at u=", formd(
        depotObject.u), " value=", depotObject.value);
  }
}
/**
#############################################################
...
road.prototype.changeTrafficLight = function(id, value) {
  ...
  var pickedTL;
  for (var i = 0;
    (!success) && (i < this.trafficLights.length); i++) {
  ...
  if (pickedTL.value === "green") {
    for (var i = 0; i < this.veh
      .length; i++) { //!!! is this.veh.length updated?
      if (this.veh[i].id === id) {
  ...
  // commands changeTrafficLight(id,"red") without (id,"green") given!
    ...
    if (!virtVehAlreadyExist) {
      for (var il = 0; il < this.nLanes; il++) {
        var virtVeh = new vehicle(1, this.laneWidth, pickedTL.u, il, 0,
          "obstacle");
        virtVeh.longModel = new ACC(0, IDM_T, IDM_s0, 0,
        IDM_b); // needed for MOBIL
        virtVeh.id = id;
...
road.prototype.removeTrafficLight = function(id) {
  ...
  console.log("in road.removeTrafficLight: id=", id,
  ...
  if (iDel === -1) console.log("road.removeTrafficLight: no id ", id,
    " found!");
