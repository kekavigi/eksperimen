...
function handleTouchStart(evt) {
  ...
  if (true) {
    ctx.beginPath();
    ctx.arc(scale * xUser, -scale * yUser, 4, 0, 2 * Math.PI,
    false); // a circle at the start
...
function pickRoadOrObject(xUser, yUser) {
  ...
  */
  if (true) {
    console.log("itime=", itime, " in pickRoadOrObject: xUser=", formd0(xUser),
      " yUser=", formd0(yUser));
  }
  //==============================================================
  ...
  if (!(typeof trafficObjs === 'undefined')) {
    ...
    var pickResults = trafficObjs.pickObject(xPixUser, yPixUser, distCrit_m *
      scale);
    console.log(
    ...
    if (pickResults[0]) {
      ...
      if (false) {
        console.log("  end pickRoadOrObject: success! picked trafficObject id=",
          trafficObject.id, " type ", trafficObject.type, " isActive=",
          trafficObject.isActive, " inDepot=", trafficObject.inDepot, " end");
      }
      return;
    }
...
function doDragging(xUser, yUser, xUserDown, yUserDown) {
  if (mousedown || touchdown) {
    ...
    distDrag = Math.sqrt(Math.pow(xUser - xUserDown, 2) + Math.pow(yUser -
      yUserDown, 2));
    if (false) {
      console.log("mousemove && mousedown: roadPicked=", roadPicked,
        " depotObjPicked=", depotObjPicked, " funnelObjPicked=",
        funnelObjPicked, " xUser=", xUser, "xUserDown=", xUserDown,
...
function handleMouseUp(evt) {
  ...
  drawSim();
  if (false) {
    console.log("  end handleMouseUp(evt):", " speedlBoxActive=",
      speedlBoxActive);
  }
}
// #########################################################
...
function finishDistortOrDropObject(xUser, yUser) {
  if (false) {
    console.log("itime=", itime, " in finishDistortOrDropObject (canvas_gui):",
      " trafficObjPicked=", trafficObjPicked, " roadPicked=", roadPicked, "");
  }
  mousedown = false;
  touchdown = false;
  ...
  if (trafficObjPicked) {
    ...
    trafficObjs.dropObject(trafficObject, network, xPixUser, yPixUser,
      distCritPix, scale);
    trafficObjPicked = false;
...
function handleClick(event) {
  ...
  if (true) {
    console.log("\n\nitime=", itime, " in handleClick: xPixUser=", formd0(
        xPixUser), " yPixUser=", formd0(yPixUser), " xUser=", formd(xUser),
      " yUser=", formd(yUser), " distDrag=", formd(distDrag));
    console.log("  handleClick: didSpeedlManip=", didSpeedlManip, " isDragged=",
      isDragged, " speedlBoxActive=", speedlBoxActive);
  }
  ...
  console.log("  handleClick: before influenceClickedVehOrTL: didSpeedlManip=",
    didSpeedlManip);
  if (!didSpeedlManip) {
    influenceClickedVehOrTL(xUser, yUser);
  }
...
function changeSpeedl(xPixUser, yPixUser) {
  console.log("\n\nitime=", itime, " in changeSpeedl (canvas_gui):",
    " speedlBoxActive=", speedlBoxActive);
  if (speedlBoxActive) {
    if ((xPixUser > speedlBoxAttr.xPixLeft) && (xPixUser < speedlBoxAttr
        .xPixLeft + speedlBoxAttr.wPix) && (yPixUser > speedlBoxAttr.yPixTop) &&
  ...
  console.log("  end changeSpeedl: traffic object of id=", speedlBoxAttr.obj.id,
    " type=", speedlBoxAttr.obj.type, " has new speed limit ", speedlBoxAttr
    .obj.value, " using image file ", speedlBoxAttr.obj.image.src);
  // a=gieskanne;
}
...
function drawSpeedlBox() {
  if (speedlBoxActive) {
    ...
    for (var i = 0; i < limits.length; i++) {
      ...
      ctx.fillText(textStr, xPixLeft + 0.01 * sizePix, yPixTop + (i + 0.7) *
        hPix / limits.length);
    }
  }
}
...
function handleDependencies() {
  ...
  if (scenarioString === "OnRamp") {
    ...
    if (true) {
      console.log("after handleDependencies: onramp: ", " ramp.veh[0].u=", ramp
        .veh[0].u, " mainRampOffset=", mainRampOffset);
    }
    ...
    console.log("after handleDependencies: offramp: offrampLastExits=",
      offrampLastExits);
    ...
    umainMerge = ramp.getNearestUof(mainroad, ramp.roadLen - lrampDev);
