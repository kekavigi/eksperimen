...
function update_v_dvdt_optical(vehicle) {
  ...
  if ((vehicle.id != 1) && (vehicle.isRegularVeh)) {
    ...
    var dv = (dt_afterLC < 0.5 * dt_LC) ? (1 - laneFraction) + 0.5 * acc_v *
      dt * dt : 1 - 0.5 * acc_v * dt * dt;
    vehicle.v = (dt_afterLC > dt_LC) ? laneEnd : laneStart + dv * (laneEnd -
      laneStart);
    // optical lateral velocity component
    vehicle.dvdt = (dt_afterLC > dt_LC) ? 0 : acc_v * dt * (laneEnd -
    laneStart);
