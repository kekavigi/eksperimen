...
IDM.prototype.copy = function(longModel) {
  ...
  this.speedmax = longModel
  .speedmax; // if engine restricts speed, speedmax<speedlimit, v0
...
IDM.prototype.calcAcc = function(s, v, vl, al) {
  ...
  var accFree = (v < v0eff) ? this.a * (1 - Math.pow(v / v0eff, 4)) : this.a *
    (1 - v / v0eff);
  var sstar = this.s0 + Math.max(0., v * this.T + 0.5 * v * (v - vl) / Math
    .sqrt(this.a * this.b));
...
function myTanh(x) {
  return (x > 50) ? 1 : (x < -50) ? -1 : (Math.exp(2 * x) - 1) / (Math.exp(2 *
    x) + 1);
}
...
ACC.prototype.calcAcc = function(s, v, vl, al) { // this works as well
  ...
  var accCAH = (vl * (v - vl) < -2 * s * al) ? v * v * al / (vl * vl - 2 * s *
    al) : al - Math.pow(v - vl, 2) / (2 * Math.max(s, 0.01)) * ((v > vl) ?
    1 : 0);
  accCAH = Math.min(accCAH, this.a);
  var accMix = (accIDM > accCAH) ? accIDM : accCAH + this.b * myTanh((accIDM -
    accCAH) / this.b);
  ...
  if (false) {
    //if(s<2){
    console.log('ACC.calcAcc:' + ' s=' + parseFloat(s).toFixed(3) + ' v=' +
      parseFloat(v).toFixed(3) + ' vl=' + parseFloat(vl).toFixed(3) +
      ' al=' + parseFloat(al).toFixed(3) + ' accFree=' + parseFloat(accFree)
      .toFixed(3) + ' accIDM=' + parseFloat(accIDM).toFixed(3) +
      ' accCAH=' + parseFloat(accCAH).toFixed(3) + ' accACC=' + parseFloat(
        accACC).toFixed(3) + ' accReturn=' + parseFloat(accReturn).toFixed(
        3));
  }
  return accReturn;
...
ACC.prototype.calcAccGiveWay = function(sYield, sPrio, v, vPrio, accOld) {
  ...
  var priorityRelevant = ((accPrioNoYield < -0.2 * this.b) && (accYield < -
    0.2 * this.b));
  var accGiveWay = priorityRelevant ? accYield : accOld;
  if (false) {
    console.log('ACC.calcAccGiveWay: sYield=', sYield, ' sPrio=', sPrio,
      ' v=', v, ' vPrio=', vPrio, '\n accPrioNoYield=', accPrioNoYield,
      ' accYield=', accYield, ' accOld=', accOld, ' accGiveWay=', accGiveWay
      );
  }
  return accGiveWay;
  //return -4;
...
MOBIL.prototype.realizeLaneChange = function(vrel, acc, accNew, accLagNew,
  ...
  if (false) {
    //if((dacc>0)&&(!toRight)){
    console.log('!!!!! positive left MOBIL LC decision!', '\n vrel=',
      parseFloat(vrel).toFixed(2), ' bSafeActual=', parseFloat(bSafeActual)
      .toFixed(2), ' acc=', parseFloat(acc).toFixed(2), ' accNew=',
      parseFloat(accNew).toFixed(2), ' bBiasRight=', parseFloat(this
        .bBiasRight).toFixed(2), ' bThr=', parseFloat(this.bThr).toFixed(2));
  }
  return (dacc > 0);
}
/**
