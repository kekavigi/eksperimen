...
function stationaryDetector(road, u, dtAggr) {
  ...
  if (this.u > road.roadLen) {
    console.log("Warning: trying to place a detector at position u=", this.u,
      " greater than the road segment length ", road.roadLen, " resetting u=",
      road.roadLen);
    this.u = road.roadLen;
  }
  ...
  this.vehNearOld = (this.u < 0.5 * this.road.roadLen) ? this.road.findLeaderAt(
    this.u) : this.road.findFollowerAt(this.u);
}
stationaryDetector.prototype.update = function(time, dt) {
  var vehNear = (this.u < 0.5 * this.road.roadLen) ? this.road.findLeaderAt(
    this.u) : this.road.findFollowerAt(this.u);
  if (vehNear.id != this.vehNearOld.id) {
    // if desired, add single-vehicle data record here
    if (false) {
      console.log("stationaryDetector.update: new single-veh", " t=", time,
        " lane=", i, " vehID=", vehNear.id);
    }
  ...
  if (time >= this.iAggr * this.dtAggr + this.dtAggr) {
    ...
    this.speedSum = 0;
    if (false) {
      console.log("\nnew aggregation:", " this.historyFlow[", this.iAggr,
        "]=", this.historyFlow[this.iAggr], " this.historySpeed[", this
        .iAggr, "]=", this.historySpeed[this.iAggr]);
    }
  }
}
...
stationaryDetector.prototype.display = function(textsize) {
  ...
  var flowStr = "Flow: " + Math.round(3600 * this.historyFlow[this.iAggr]) +
    " veh/h";
  var speedStr = "Speed: " + ((this.historyFlow[this.iAggr] > 0) ? Math.round(
    3.6 * this.historySpeed[this.iAggr]) : "--") + " km/h";
  ...
  var toRight_axis = -0.5 * roadWidth - (2.2 + 1.8 * Math.abs(sphi)) *
    laneWidth;
  ...
  var yCenterPix1 = -scale * this.road.traj_y(this.u - 0.5 *
  detLineDist); //minus!!
  ...
  ctx.fillRect(-0.5 * wPix, -0.5 * lPix, wPix,
  lPix); // second line double det
  // the textbox
  ...
  ctx.fillRect(xPixCenter - 0.5 * boxWidth, yPixCenter - 0.5 * boxHeight,
    boxWidth, boxHeight);
  ctx.fillStyle = "rgb(0,0,0)";
  ctx.fillText(flowStr, xPixCenter - 0.46 * boxWidth, yPixCenter - 0.1 *
    boxHeight);
  ctx.fillText(speedStr, xPixCenter - 0.46 * boxWidth, yPixCenter + 0.4 *
    boxHeight);
}
