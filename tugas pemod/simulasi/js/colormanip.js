...
function hslToRgb(h, s, l) {
  ...
  if (myVersion) {
    ...
    var r_smax = Math.max(0, 1 - 6 * Math.pow(h, 2), 1 - 9 * Math.pow(h - 1,
    2));
...
function colormapSpeed(v, vmin_col, vmax_col, vehType, isEgo, time, isOpaque) {
  ...
  var vrel_nl = (vrel <= 0.5) ? 2 * Math.pow(vrel, 2) : 1 - 2 * Math.pow(vrel -
    1, 2)
  ...
  if (isEgo) {
    var lightsOn = ((Math.floor(1000 * time)) % dt_blink_ms < 0.5 *
    dt_blink_ms);
    a = (lightsOn) ? 0.95 : 0.2;
  }
  if (isOpaque) {
...
function displayColormap(xCenterPix, yCenterPix, widthPix, heightPix,
  ...
  ctx.setTransform(1, 0, 0, 1, xCenterPix - 0.5 * widthPix, yCenterPix - 0.5 *
    heightPix);
  ...
  for (var i = 0; i < nvals; i++) {
    var xLeft = 0;
    var yTop = i * hBox;
    var val = vmin_colDisplay + i / (nvals - 1) * (vmax_colDisplay -
      vmin_colDisplay);
    ctx.fillStyle = colormapSpeed(val, vmin_colMap, vmax_colMap, 'car', false,
      0, true);
  ...
  for (var i = 0; i < nlegend; i++) {
    var v = vmin_colMap + i * (vmax_colDisplay - vmin_colDisplay) / (nlegend -
      1);
