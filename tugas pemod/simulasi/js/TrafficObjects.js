...
function TrafficObjects(canvas, nTL, nLimit, xRelDepot, yRelDepot, nRow, nCol) {
  ...
  this.nObst = Math.max(0, this.n - nTL -
  nLimit); // nTL,nLimit, not this,nTL,...
  this.nObstMax = 10;
  ...
  this.trafficObj = [];
  var initSpeedInd = [6, 8, 10, 0, 12, 3, 4, 5, 1,
  2]; // speed 60 km/h,80,100,free..
  for (var i = 0; i < this.n; i++) {
    ...
    var img = (isTL) ? this.imgTLred : (isSpeedl) ? this.imgSpeedlRepo[
      initSpeedInd[iSpeed]] : this.imgObstRepo[iObst];
    console.log('TrafficObjects cstr: i=', i, ' img=', img, ' iObst=', iObst);
    //#################################################################
    ...
    this.trafficObj[i] = {
      ...
      , type: (isTL) ? 'trafficLight' : (isSpeedl) ?
        'speedLimit' : 'obstacle'
      , image: (isTL) ? this.imgTLred : (isSpeedl) ? this.imgSpeedlRepo[
        initSpeedInd[iSpeed]] : this.imgObstRepo[iObst]
    ...
    if ((this.trafficObj[i].type == 'speedLimit') && (this.trafficObj[i]
        .value == 0)) {
  ...
  if (true) {
    ...
    for (var i = 0; i < this.trafficObj.length; i++) {
      console.log('TrafficObjects cstr: i=', i, ' value=', this.trafficObj[i]
        .value, ' type=', this.trafficObj[i].type, ' id=', this.trafficObj[i]
        .id, ' imgfile=', this.trafficObj[i].image.src, ' isActive=', this
        .trafficObj[i].isActive);
    }
  }
...
TrafficObjects.prototype.calcDepotPositions = function(canvas) {
  this.sizeCanvas = Math.min(canvas.width, canvas.height);
  this.wPix = this.sizeRel * this
  .sizeCanvas; // diameter [pix] of traffObj signs
  this.hPix = this.wPix;
  ...
  for (var i = 0; i < this.n; i++) {
    ...
    this.trafficObj[i].xPixDepot = xPixDepotCenter + (this.wPix + gapPix) * (
      icol - 0.5 * (this.nCol - 1));
    this.trafficObj[i].yPixDepot = yPixDepotCenter + (this.hPix + gapPix) * (
      irow - 0.5 * (this.nRow - 1));
...
TrafficObjects.prototype.draw = function() {
  ...
  for (var i = 0; i < this.trafficObj.length; i++) {
    ...
    // ==============================================
    if ((obj.isActive) && ((obj.type === 'trafficLight') || (obj.type ===
        'speedLimit'))) {
      var obj = this.trafficObj[i];
      var road = obj.road;
      ...
      if (this.active_drawTopSign) { // draw active sign above the road
        ...
        ctx.drawImage(obj.image, -0.5 * wPixActive, -hPixActive, wPixActive,
          hPixActive);
        obj.xPixSign1 = xPix; // save pixel positions of
        obj.yPixSign1 = yPix - 0.8 *
        hPixActive; // light centers for later picking
      }
      ...
      if (false) {
        console.log('TrafficObjects.draw active obj: i=', i, ' type=', obj
          .type, ' obj.u=', obj.u, ' obj.xPixSign1=', obj.xPixSign1,
    ...
    if (!obj.isActive) {
      ...
      if (false) {
        ...
        console.log('time=', time,
          ' in TrafficObjects.draw: passive objects: id=', obj.id,
          ' value=', obj.value,
          //' fname=',obj.image.src,
          //' xPix=',formd(obj.xPix),
          //' yPix=',formd(obj.yPix),
...
*/
TrafficObjects.prototype.selectByUser = function(xPixUser, yPixUser,
  distCritPix) {
  ...
  var i_opt = -1;
  for (var i = 0; i < this.trafficObj.length; i++) {
    var dist2 = Math.pow(xPixUser - this.trafficObj[i].xPix, 2) + Math.pow(
      yPixUser - this.trafficObj[i].yPix, 2);
    if (dist2 < dist2_min) {
      dist2_min = dist2;
      i_opt = i;
    }
  ...
  if (false) {
    if (success) {
      console.log('  success! type=', trafficObjreturn.type, ' isActive=',
        trafficObjreturn.isActive, ' xPixUser=', formd0(xPixUser),
        ' yPixUser=', formd0(yPixUser), ' xPix=', formd0(this.trafficObj[
          i_opt].xPix), ' yPix=', formd0(this.trafficObj[i_opt].yPix), 'end'
        );
    } else {
      console.log('  no success', ' nearest object has type', this.trafficObj[
          i_opt].type, ' xPixUser=', formd0(xPixUser), ' yPixUser=', formd0(
          yPixUser), ' xPix=', formd0(this.trafficObj[i_opt].xPix),
        ' yPix=', formd0(this.trafficObj[i_opt].yPix), 'end');
    }
  }
...
TrafficObjects.prototype.selectById = function(id) {
  ...
  var i_success = -1;
  for (var i = 0;
    (i < this.trafficObj.length) && (!success); i++) {
...
TrafficObjects.prototype.activate = function(obj, road, u) {
  ...
  obj.isActive = true;
  if (!(typeof u ===
    'undefined')) { // external setting; must take care of all
    obj.u = u;
...
TrafficObjects.prototype.deactivate = function(obj) {
  ...
  if (obj.isActive) {
    if (obj.type === 'trafficLight') {
      console.log('TrafficObjects.deactivate');
      road.removeTrafficLight(obj.id);
    }
    if (obj.type === 'obstacle') {
      road.removeObstacle(obj.id);
    }
...
*/
TrafficObjects.prototype.pickObject = function(xPixUser, yPixUser,
distCritPix) {
...
*/
TrafficObjects.prototype.dropObject = function(obj, network, xPixUser, yPixUser,
  distCritPix, scale) {
  console.log('itime=', itime, ' in TrafficObjects.dropObject: obj.id=', obj
    .id, ' obj.xPix=', obj.xPix, ' network[0].roadID=', network[0].roadID);
  ...
  if (success && obj.type === 'obstacle') {
    obj.u += du;
    obj.lane = Math.round(Math.max(0, Math.min(road.nLanes - 1,
      dropInfoNearest[2])));
  }
  // update pixel coordinates to 'snapped' objects for later picking
  if (success) {
    console.log('  success! roadID=', obj.road.roadID, ' obj.u=', obj.u,
      ' obj.lane=', obj.lane);
  ...
  if (true) {
    console.log('  end TrafficObjects.dropObject: success=', success,
      ' nearest roadID=', road.roadID, ' road.roadLen=', formd(road
      .roadLen), ' obj.id=', obj.id, ' obj.u=', formd(obj.u), ' obj.xPix=',
      formd0(obj.xPix), '');
  }
} // dropObject
//#############################################################
...
TrafficObjects.prototype.selectSignOrTL = function(xPixUser, yPixUser) {
  if (false) {
    console.log('in TrafficObjects.selectSignOrTL:', ' xPixUser=', xPixUser,
      ' yPixUser=', yPixUser);
  }
  ...
  var objFound = 'void';
  for (var i = 0;
    (!success) && (i < this.trafficObj.length); i++) {
    var obj = this.trafficObj[i];
    if ((!obj.isDragged) && ((obj.type === 'trafficLight') || (obj.type ===
        'speedLimit'))) {
      ...
      if (false) {
        console.log('selectSignOrobj: obj.id=', obj.id, ' obj.xPix=', formd0(
            obj.xPix), ' obj.yPix=', formd0(obj.yPix), ' obj.xPixSign1=',
          formd0(obj.xPixSign1), ' obj.xPixSign2=', formd0(obj.xPixSign2),
          ' distPix=', formd0(distPix), ' distPix1=', formd0(distPix1),
          ' distPix2=', formd0(distPix2), ' distPixCrit=', formd0(
            distPixCrit), ' success=', success);
      }
    }
  }
...
TrafficObjects.prototype.setTrafficLight = function(obj, value) {
  ...
  if (false) {
    if (obj.isActive) {
      console.log('setTrafficLight:  id=', obj.id, ' road ID=', obj.road
        .roadID);
...
*/
TrafficObjects.prototype.changeTrafficLightByUser = function(xPixUser,
yPixUser) {
    ...
    if (obj.type === 'trafficLight') {
      ...
      if (false) {
        console.log('end TrafficObjects.changeTrafficLightByUser: ',
          ' changed traffic light', ' to ', obj.value, ' at u=', obj.u);
        if (obj.isActive) {
...
TrafficObjects.prototype.zoomBack = function() {
  var relDisplacementPerCall =
  0.02; // zooms back as attached to a rubber band
  var pixelsPerCall = relDisplacementPerCall * this.sizeCanvas;
  for (var i = 0; i < this.trafficObj.length; i++) {
    var obj = this.trafficObj[i];
    if ((!obj.isActive) && (!obj.inDepot) && (!obj.isDragged) && (!obj
        .isPicked)) {
      ...
      if (true) {
        console.log('TrafficObjects.zoomBack: i=', i, ' obj.xPix=', obj.xPix,
          ' obj.xPix=', obj.xPix, ' this.trafficObj[i].xPix=', this
          .trafficObj[i].xPix);
      }
    }
  }
...
TrafficObjects.prototype.writeObjects = function(onlyTL) {
  ...
  console.log('itime=', itime, ' in TrafficObjects.writeObjects:', ' justTL=',
    justTL, ':');
  for (var i = 0; i < this.trafficObj.length; i++) {
    if ((!justTL) || (this.trafficObj[i].type === 'trafficLight')) {
      var obj = this.trafficObj[i];
      console.log('  id=', obj.id, ' type=', obj.type, ' roadID=', obj.road
        .roadID, ' u=', formd(obj.u), ' lane=', formd(obj.lane), ' value=',
        obj.value,
